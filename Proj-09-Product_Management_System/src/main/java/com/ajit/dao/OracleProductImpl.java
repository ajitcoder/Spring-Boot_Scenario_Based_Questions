...
public class OracleProductImpl implements IProductDAO {
	...
	private static final String INSERT_PRODUCT = "INSERT INTO PRODUCTS  (PID,PNAME,PRICE) VALUES (?,?,?)";
	private static final String SELECT_PRODUCT = "SELECT * FROM PRODUCTS";
	private static final String UPDATE_PRODUCT = "UPDATE PRODUCTS SET PRICE=? WHERE PID=?";
	private static final String DELETE_PRODUCT = "DELETE FROM PRODUCTS WHERE PID=?";
	...
	public int updateProductPrice(int pId, double newPrice) throws Exception {
		...
		try(Connection con = ds.getConnection();
			...
			updatedSalary= pstmt.executeUpdate();
